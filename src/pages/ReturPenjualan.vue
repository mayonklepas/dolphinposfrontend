            ...
            <table class="table-minimal">
              <thead>
                <tr>
                  <th
                    class="pointer"
                    v-on:click="sortData('kodePenjualanMaster')"
                  >
                    Kode
                  </th>
                  <th class="pointer" v-on:click="sortData('tanggalPenjualan')">
                    Tanggal
                  </th>
                  ...
                  <th class="pointer" v-on:click="sortData('penjualanDetail')">
                    Total
                  </th>
              ...
              <tbody>
                <tr v-for="x in resData.content" :key="x.id">
                  <td>{{ x.kodeReturPenjualanMaster | limitText }}</td>
                  <td>{{ x.tanggalReturPenjualan | formatDate }}</td>
                  <td>
                  ...
                  <td>
                    ...
                    <!-- {{
                      returPenjualanDetailTotal(x.returPenjualanDetail, x.disc, x.tax)
                        | formatNumber
                    }}-->
                  </td>
                  <td>
...
export default {
  ...
  methods: {
    ...
    deleteData() {
      this.showProgress();
      this.$axios
        .delete(this.$url + "retur-penjualan/deletedata/" + this.id)
        .then((response) => {
    ...
    returPenjualanDetailTotal(returPenjualanDetail, disc, tax) {
      let total = 0;
      let taxsimple = tax / 100;
      for (let i = 0; i < returPenjualanDetail.length; i++) {
        let hargaJual = returPenjualanDetail[i].hargaJualJual;
        let jumlahJualRetur = returPenjualanDetail[i].jumlahJualRetur;
        let disc = returPenjualanDetail[i].disc;
        total = total + (hargaJual * jumlahJualRetur - disc * jumlahJualRetur);
      }
