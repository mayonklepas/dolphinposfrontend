<template>
  <div class="TransaksiJurnalCashReceipt">
    <div class="container">
      <div class="row" style="margin-bottom:5px;">
        <div class="col-12 d-flex justify-content-between">
          <label class="title">
            <i class="fa fa-table" style="color:#313131 !important"></i> Input Jurnal
            {{ setTextTipeJurnal() }}</label>
          <button type="button" class="float-right" v-on:click="backToJurnalUmum">
      ...
      <div class="row">
        ...
        <div class="col-2">
          <label>Akun</label><br />
          <select name="" v-model="idAkunKeuanganDrop">
            <option v-for="x in resDataAkunKeuanganDrop" :key="x.id" v-bind:value="x.id">{{
              x.namaAkunKeuangan
              }}</option>
          </select>
        </div>
              ...
              <tbody>
                <tr v-for="(x, index) in resData" :key="index">
                  ...
                  <td style="width:150px;">
                    <input type="text" v-model="x.akunKeuangan.kodeAkunKeuangan"
                      v-on:keyup.enter="findAkunKeuanganByKode(index)" />
                  </td>
                  <td style="width:250px;">
                    <input type="text" v-model="x.akunKeuangan.namaAkunKeuangan" />
                  </td>
                  <td>
                    <input type="text" v-model="x.deskripsi" />
                  </td>
                  <td style="width:150px;">
                    <input type="text" v-model="x.jumlah" v-on:keyup="formatJumlah(index)" />
                  </td>
      ...
      <div class="row">
        <div class="col-6">
          <button type="button" v-on:click="addData"><i class="fa fa-plus"></i> Tambah Data</button>
        ...
        <div class="col-6">
          <div class="row">
            ...
            <div class="col-3">
              <span class="float-right">{{ totalJumlah | formatNumber }}</span>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="row">
              <div class="col-6">
                <div class="pagination">
                  <a v-on:click="prevDataAkunKeuangan" class=""><i
                      class="fa fa-chevron-left tx-small"></i> Prev</a>
                  ...
                  <a v-on:click="nextDataAkunKeuangan" class="">Next <i
                      class="fa fa-chevron-right tx-small"></i></a>
                </div>
              </div>
              <div class="col-4">
  ...
  export default {
    ...
    data() {
      return {
        ...
        deskripsi: "",
        idAkunKeuanganDrop: "",
        tipeJurnal: this.$route.query.tipejurnal,
        tipe: this.$route.query.tipe,
        indexPosition: 0,
        totalJumlah: 0,
        resDataAkunKeuanganDrop: [],
        //Akun
        resDataAkunKeuangan: {},
    ...
    methods: {
      setTextTipeJurnal() {
        let namatipe = "";
        if (this.tipeJurnal == 0) {
          namatipe = "Penerimaan";
        } else {
          namatipe = "Pengeluaran";
        }
        return namatipe;
      },
      showProgress() {
      ...
      loadData() {
        if (this.tipe == 0) {
          ...
          let cdate = new Date();
          this.tanggalJurnal = cdate.toISOString().slice(0, 10);
          this.tanggalRef = cdate.toISOString().slice(0, 10);
        } else {
          console.log(this.paramData);
          this.tanggalJurnal = this.paramData.tanggalJurnal;
          this.noRef = this.paramData.noRef;
          this.tanggalRef = this.paramData.tanggalRef;
          this.resData = this.paramData.jurnalUmumDetail;
      ...
      saveData() {
        ...
        if (this.tanggalJurnal !== "" && this.noRef !== "" && this.tanggalRef !== "") {
          let idMaster = "";
          if (this.resData.length > 0) {
            ...
            if (this.tipe == 0) {
              ...
              idMaster = this.paramData.id;
              dest = "updatedata/" + this.paramData.id;
            }
            let postdatamaster = {
            ...
            if (this.tipeJurnal == 1) {
              postdatadetail.push({
                idJurnalMaster: idMaster,
                tanggalJurnal: this.tanggalJurnal,
                idAkunKeuangan: this.idAkunKeuanganDrop,
                deskripsi: this.deskripsi,
                debit: 0,
                kredit: this.totalJumlah,
                idPengguna: this.$idPengguna,
                idOutlet: this.$idOutlet,
                urutan: 1,
              });
              for (let i = 0; i < this.resData.length; i++) {
                postdatadetail.push({
                  ...
                  debit: this.stdNum(this.resData[i].jumlah),
                  kredit: 0,
                  idPengguna: this.$idPengguna,
                  idOutlet: this.$idOutlet,
                  urutan: i + 2,
              ...
              for (let i = 0; i < this.resData.length; i++) {
                postdatadetail.push({
                  ...
                  deskripsi: this.resData[i].deskripsi,
                  debit: 0,
                  kredit: this.stdNum(this.resData[i].jumlah),
                  idPengguna: this.$idPengguna,
                  idOutlet: this.$idOutlet,
                  urutan: i + 2,
                });
            ...
            this.$axios
              .post(this.$url + "jurnalumum/" + dest, JSON.stringify(postdata), {
                headers: {
            ...
            this.modalInfoText = "Operasi ditolak, data detail tidak boleh kosong";
            this.modalInfo = true;
          }
        } else {
      ...
      findAkunKeuanganByKode(index) {
        ...
        if (keyword == "") {
          ...
          this.$axios
            .get(this.$url + "akunkeuangan/getdatabykode/" + keyword + "/" + this.$idOutlet + "/")
            .then((response) => {
              console.log(response);
      ...
      calculateJumlah() {
        ...
        for (let index = 0; index < this.resData.length; index++) {
          this.totalJumlah = this.totalJumlah + this.stdNum(this.resData[index].jumlah);
        }
      },
      closeModalInfo() {
        this.modalInfoText = "";
      ...
      pilihModalAkunKeuangan(idAkunKeuangan, kodeAkunKeuangan, namaAkunKeuangan) {
        ...
        if (cekindex < 0) {
          ...
          this.resData[this.indexPosition].akunKeuangan.kodeAkunKeuangan = kodeAkunKeuangan;
          this.resData[this.indexPosition].akunKeuangan.namaAkunKeuangan = namaAkunKeuangan;
      ...
      loadDataAkunKeuanganDrop() {
        this.$axios
          .get(this.$url + "akunkeuangan/getdataall/" + this.$idOutlet)
          .then((response) => {
            this.resDataAkunKeuanganDrop = response.data;
            console.log(response);
          });
      },
      //akun keuangan
      ...
      formatJumlah(index) {
        this.calculateJumlah();
        this.resData[index].jumlah = this.fmNum(this.resData[index].jumlah);
      },
    },

    mounted() {
      this.loadData();
      this.loadDataAkunKeuanganDrop();
    },
  };
</script>