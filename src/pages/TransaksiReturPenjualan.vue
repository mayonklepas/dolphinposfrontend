  ...
  <div class="Transaksi Penjualan">
    <div class="container">
      <div class="row" style="margin-bottom: 5px">
        <div class="col-12 d-flex justify-content-between">
          <label class="title">
            ...
            Transaksi Retur Penjualan</label>
          <div>
            ...
            <button type="button" class="float-right" v-on:click="backToReturPenjualan">
      ...
      <div class="row">
        <div class="col-2">
          <label>Tanggal</label>
          <input type="date" v-model="tanggalReturPenjualan" />
        </div>
        <div class="col-2">
          <label>No. Invoice</label>
          <div>
            <input type="text" v-model="kodeReturPenjualanMaster" style="width: 78%;" />
        ...
        <div class="col-2">
          <label v-if="isAccMode == true">Kredit</label>
              ...
              <tbody>
                <tr v-for="(x, index) in resData" :key="index">
                  ...
                  <td style="width: 120px">
                    <input type="number" style="width:60px" v-model.number="x.jumlahJualRetur" v-on:keyup="calculatetotalrow(index)"
                      v-on:click="calculatetotalrow(index)" />
                      {{x.satuanBarang}}
                  </td>
          ...
          <div class="row">
            ...
            <div class="col-4">
              <span class="float-right">{{
                  totalReturPenjualan | formatNumber
              }}</span>
            </div>
            <div class="col-8">
...
export default {
  ...
  data() {
    return {
      ...
      resDataAkunKeuangan: [],
      resDataKartuKontak: {},
      tanggalReturPenjualan: "",
      kodeReturPenjualanMaster: "",
      idKartuKontak: this.$settings.defaultCustomer,
      idAkunKeuangan: this.$settings.akunReturPenjualanKredit.id,
      idAkunKeuanganDebit: this.$settings.akunReturPenjualanDebit.id,
      deskripsi: "",
  ...
  methods: {
    ...
    loadData() {
      if (this.tipe == 0) {
        let cdate = new Date();
        this.tanggalReturPenjualan = cdate.toISOString().slice(0, 10);
        this.addData();
      } else {
        this.$axios
          .get(
            this.$url +
            "retur-penjualan/getdatabyid/" +
            this.$idOutlet +
            "/" +
            this.idMaster
          )
          .then((response) => {
            ...
            let jualDetail = data.returPenjualanDetail;
            ...
            this.tanggalReturPenjualan = data.tanggalReturPenjualan;
            this.kodeReturPenjualanMaster = data.kodeReturPenjualanMaster;
            this.idKartuKontak = data.idKartuKontak;
            this.idAkunKeuangan = data.idAkunKeuangan;
            this.idAkunKeuanganDebit = data.idAkunKeuanganDebit;
            this.deskripsi = data.deskripsi;
            this.tax = data.tax;
            this.disc = data.disc;
            this.resData = data.returPenjualanDetail;
    ...
    saveData() {
      ...
      if (
        this.tanggalReturPenjualan !== "" &&
        this.kodeReturPenjualanMaster !== "" &&
        this.idKartuKontak !== "" &&
        this.idAkunKeuangan !== "" &&
        this.idAkunKeuanganDebit !== ""
      ) {
        let idMaster = "";
        let statusReturPenjualan = 1;
        if (this.resData.length > 0) {
          ...
          if (this.tipe == 0) {
            dest = "adddata";
            idMaster = "";
            statusReturPenjualan = 3;
          } else {
            idMaster = this.idMaster;
            dest = "updatedata/" + this.idMaster;
            statusReturPenjualan = 2;
          }
          let postdatamaster = {
            kodeReturPenjualanMaster: this.kodeReturPenjualanMaster,
            tanggalReturPenjualan: this.tanggalReturPenjualan,
            status: statusReturPenjualan,
          ...
          for (let i = 0; i < this.resData.length; i++) {
            postdatadetail.push({
              tanggalTransaksi: this.tanggalReturPenjualan,
              idReturPenjualanMaster: idMaster,
              ...
              jumlahJualRetur: this.resData[i].jumlahJualRetur,
        ...
        this.$axios
          ...
          .then((response) => {
            ...
            if (response.data !== null) {
              ...
              this.resData[index].jumlahJualRetur = 1;
    ...
    calculatetotalrow(index) {
      ...
      let jumlah = this.stdNum(this.resData[index].jumlahJualRetur);
    ...
    calculatetotalall() {
      let totalReturpenjualanraw = 0;
      for (let i = 0; i < this.resData.length; i++) {
        totalReturpenjualanraw = totalReturpenjualanraw + this.resData[i].total;
      }
      this.totalReturPenjualan = totalReturpenjualanraw;
    },
    calculategrandtotal() {
      let persentax =
        (this.tax / 100) *
        (parseFloat(this.stdNum(this.totalReturPenjualan)) - this.disc);
      let totalplusdisc =
        parseFloat(this.stdNum(this.totalReturPenjualan)) - this.disc;
    ...
    pilihModalBarang(
      ...
      this.resData[this.indexPosition].jumlahJualRetur = 1;
    ...
    formatJual(index, val) {
      this.resData[index].hargaJualJual = this.fmNum(val);
    },
    /*formatJual(index) {
        this.resData[index].hargaJualJual = this.fmNum(this.resData[index].hargaJualJual);
      },*/
    formatDisc(index) {
    ...
    generateInvCode() {
      ...
      this.$axios
        .get(
          ...
          "report/getinvoice/" +
          this.$idOutlet +
          "/retur_penjualan_master"
        )
        .then((response) => {
          this.kodeReturPenjualanMaster = response.data.content
          console.log(response);
          this.closeProgress();
        });
    }
