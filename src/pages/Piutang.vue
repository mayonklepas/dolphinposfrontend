...
export default {
  ...
  data() {
    return {
      ...
      tipeCode: 0,
      idKartuKontak: "",
      idAkunKeuangan: this.$settings.akunPiutangKredit.id,
      idAkunKeuanganDebit: this.$settings.akunPiutangDebit.id,
      deskripsi: "",
      jumlah: "",
      paging: [],
      pageno: 0,
  ...
  methods: {
    ...
    loadDataKartuKontak() {
      this.$axios
        .get(this.$url + "kartukontak/getdatabytipe/" + this.$idOutlet +"/1?keyword=")
        .then((response) => {
    ...
    simpanData() {
      if (
        ...
        if (this.saveTipe == 0) {
          this.$axios
            .post(this.$url + "piutang/adddata/" + this.$idOutlet, JSON.stringify(postdata), {
              headers: {
          ...
          this.$axios
            .put(this.$url + "piutang/updatedata/" + this.id, JSON.stringify(postdata), {
              headers: {
                "Content-Type": "application/json",
              },
            })
    ...
    editData(id, tanggal, idKartuKontak,idAkunKeuangan,idAkunKeuanganDebit, deskripsi, jumlah) {
      this.id = id;
    ...
    deleteData() {
      this.showProgress();
      this.$axios.delete(this.$url + "piutang/deletedata/" + this.id).then((response) => {
        this.closeProgress();
    ...
    goDetail(id) {
      this.$router.push({
        ...
        query: { tipe: this.tipe, idPiutang: id },
      });
    },
    changeAccMode() {
      if (this.isAccMode) {
