    ...
    <div class="container">
      <div class="row">
        <div class="col-12 d-flex justify-content-between">
          <label class="title">
            <i class="fa fa-cart-plus" style="color:#313131 !important"></i> Pembelian</label>
        ...
        <div class="col-12">
          <div class="table-box">
            <table class="table-minimal">
              <thead>
                <tr>
                  <th class="pointer" v-on:click="sortData('kodePembelianMaster')">Kode</th>
                  <th class="pointer" v-on:click="sortData('tanggalPembelian')">Tanggal</th>
                  <th class="pointer" v-on:click="sortData('status')">Status</th>
                  <th class="pointer" v-on:click="sortData('deskripsi')">Deskripsi</th>
                  <th class="pointer" v-on:click="sortData('pembelianDetail')">Total</th>
                  <th class="pointer">Action</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="x in resData.content" :key="x.id">
                  <td>{{ x.kodePembelianMaster | limitText }}</td>
                  <td>{{ x.tanggalPembelian | formatDate }}</td>
                  <td>
                  ...
                  <td>
                    <!-- {{ pembelianDetailTotal(x.pembelianDetail, x.disc, x.tax) | formatNumber }}-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteData() {
        this.showProgress();
        this.$axios.delete(this.$url + "pembelian/deletedata/" + this.id).then((response) => {
      ...
      pembelianDetailTotal(pembelianDetail, disc, tax) {
        let total = 0;
        let taxsimple = tax / 100;
        for (let i = 0; i < pembelianDetail.length; i++) {
          let hargaBeli = pembelianDetail[i].hargaBeliBeli;
          let jumlahBeli = pembelianDetail[i].jumlahBeli;
          let disc = pembelianDetail[i].disc;
          total = total + (hargaBeli * jumlahBeli - disc * jumlahBeli);
        }
